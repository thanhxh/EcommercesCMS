...
public class WebUI {
    ...
    public void verifyEquals (Object actual, Object expected, By by){
        waitForPageLoaded();
        sleep(STEP_TIME);
        LogUtils.info("Verify equals: " + actual + " and " + expected);
        //ExtentTestManager.logMessage(Status.PASS, "Verify equals: " + actual + " and " + expected);
    ...
    @Step("Press ENTER on keyboard with {0}")
    ...
    @Step("Press F11 on keyboard with {0}")
    ...
    @Step("Press Ctrl + A on keyboard ")
    public static boolean pressCTRL_A() {
        try {
            Actions actions = new Actions(DriverManager.getDriver());
    ...
    @Step("Press Paste file with Ctrl + V on keyboard")
    public static boolean pressPASTE() {
        try {
            Robot robot = new Robot();
